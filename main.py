...
def get_args_parser():
    ...
    parser.add_argument('--set_cost_sub_class', default=1, type=float,
                        help="Object class coefficient in the matching cost")
    parser.add_argument('--set_cost_obj_class', default=1, type=float,
                        help="Object class coefficient in the matching cost")
    parser.add_argument('--set_cost_verb_class', default=1, type=float,
                        help="Verb class coefficient in the matching cost")
    parser.add_argument('--set_cost_hoi', default=0.5, type=float,
    ...
    parser.add_argument('--sub_loss_coef', default=1, type=float) ##new
    ...
    parser.add_argument('--hoi_path', default='data',type=str)
...
def main(args):
    ...
    #                              drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
    # data_loader_val = DataLoader(dataset_val, batch_sampler=batch_sampler_train,collate_fn=utils.collate_fn, num_workers=args.num_workers)
    ...
    best_performance = 0
    ##new
    recognition = ivtmetrics.Recognition(num_class=100)
    detect = ivtmetrics.Detection(num_class=100)
    detect_i = ivtmetrics.Detection(num_class=100)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        detect.reset_global()
        detect_i.reset_global()
        recognition.reset_global()
        mAPi.reset_global()
        test_stats_ivt, test_stats_i,test_stats_rec_ivt, test_stats_rec_i, test_ivt, test_i = evaluate_hoi(args.dataset_file, model, postprocessors, 
                                data_loader_val, criterion, args.subject_category_id, device, args, detect, recognition, mAPi, detect_i)
        
        performance_ivt = test_stats_ivt['mAP']

        if performance_ivt > best_performance:
            ...
            best_performance = performance_ivt

        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     **{f'test_ivt_{k}': v for k, v in test_stats_ivt.items() if k not in ['AP', 'Rec', 'Pre']},
                     **{f'test_i_{k}': v for k, v in test_stats_i.items() if k not in ['AP', 'Rec', 'Pre']},
                     **{f'test_rec_ivt_{k}': v for k, v in test_stats_rec_ivt.items() if k not in ['AP', 'Rec', 'Pre']},
                     **{f'test_rec_i_{k}': v for k, v in test_stats_rec_i.items() if k not in ['AP', 'Rec', 'Pre']},
                     **{f'test1_ivt_{k}': v for k, v in test_ivt.items() if k not in ['AP', 'Rec', 'Pre']},
                     **{f'test1_i_{k}': v for k, v in test_i.items() if k not in ['AP', 'Rec', 'Pre']},
                     'epoch': epoch,
