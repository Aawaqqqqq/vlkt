...
class HICOEvaluator():
    def __init__(self, preds, gts, rare_triplets, non_rare_triplets, correct_mat, args):
        ...
        for index, img_preds in enumerate(preds):
            ...
            obj_scores = img_preds['obj_scores'] *  img_preds['obj_scores'] # [100, 22]
            # img_preds['hoi_scores'] #[100,100]
            # hoi_scores #[100,100]
            ...
            topk_hoi_scores = top_k(list(hoi_scores), self.max_hois) #list [100]
            topk_indexes = np.array([np.where(hoi_scores == score)[0][0] for score in topk_hoi_scores]) #[100]
        ...
        for i, img_gts in enumerate(gts):
            ...
            # bbox_anns # len=2 0:{'bbox': [373.0, 412.0, 442.0, 530.0], 'category_id': 2}
            ...
            hoi_anns = [{'subject_id': hoi[0], 'object_id': hoi[1],
                         'category_id': self.hico_triplet_labels.index((hoi[2], bbox_anns[hoi[1]]['category_id']))} # verb, object(0)
