...
class HungarianMatcherHOI(nn.Module):
    ...
    def __init__(self, cost_sub_class: float = 1, cost_obj_class: float = 1, cost_verb_class: float = 1, cost_bbox: float = 1,
    ...
    def forward(self, outputs, targets):
        ...
        tgt_obj_labels = torch.cat([v['obj_labels'] for v in targets]) #torch.Size([4])
        tgt_obj_labels = tgt_obj_labels - 6
        out_obj_prob = outputs['pred_obj_logits'].flatten(0, 1).softmax(-1) #torch.Size([400, 15])
        cost_obj_class = -out_obj_prob[:, tgt_obj_labels] # torch.Size([400, 4]) 
        
        tgt_sub_labels = torch.cat([v['sub_labels'] for v in targets]) #torch.Size([4])
        tgt_sub_labels = tgt_sub_labels 
        out_sub_prob = outputs['pred_sub_logits'].flatten(0, 1).softmax(-1) #torch.Size([400, 15])
        cost_sub_class = -out_sub_prob[:, tgt_sub_labels] # torch.Size([400, 4])
        ...
        C = self.cost_hoi_class * cost_hoi_class + self.cost_bbox * cost_bbox + \
            self.cost_giou * cost_giou + self.cost_obj_class * cost_obj_class + self.cost_sub_class * cost_sub_class
...
def build_matcher(args):
    return HungarianMatcherHOI(cost_sub_class=args.set_cost_sub_class, cost_obj_class=args.set_cost_obj_class,
                               cost_verb_class=args.set_cost_verb_class,cost_bbox=args.set_cost_bbox, 
                               cost_giou=args.set_cost_giou,cost_hoi_class=args.set_cost_hoi)